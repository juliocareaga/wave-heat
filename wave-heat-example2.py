...
if modeltype == 'W':
    ...
    kk  = lambda ss: -(betaac - 1.0)/(rho*qq(ss))*lnr    
...
###=============================================================================================        
frec_save = 1
addfolder = "Example2/" + modeltype + "/"
namesim   = "Example2_" + modeltype + labelin
listvecs  = [] 
if True:
    ...
    tag_subdomains, mark_subdomains = subdomtag(domain, domain.topology.dim,
                                     [lambda x: (x[1] >= 0.03)*(x[1] <= 0.05)*(x[0] >= -0.02)*(x[0] <= 0.02)], [1] )
    dxA = ufl.Measure('dx', domain, subdomain_data = tag_subdomains)
    ...
    death_tissue = fem.Function(Qh)
    
    onesfun = fem.Function(Qh)
    ...
    if True:
        ...
        pp.interpolate(   lambda x:   initial_p(x[0],x[1]))
        dp.interpolate(   lambda x:  initial_dp(x[0],x[1]))
        
        #theta = Ritz_projection(theta_ex, Qh, Qh, bcHeat)
        #pp    = Ritz_projection(pp_ex, Hh, Hh, bcWave)
        #dp    = Ritz_projection(dp_ex, Hh, Hh, bcWave)
        ...
        with io.XDMFFile(domain.comm, "SIM/" + addfolder + namesim + ".xdmf", "w") as xdmf:
            xdmf.write_mesh(domain)
            
            pp2.name    = "pressure";           xdmf.write_function(pp2,inc)
            theta.name  = "temperature";        xdmf.write_function(theta,inc)
            death_tissue.name = "death_tissue"; xdmf.write_function(death_tissue,inc) 

            tk = 0
            
            death_tissue_A = np.array([0])
            death_tissue_O = np.array([0])
            death_tissue_F = np.array([0])
            
            onesfun.x.array[:] = 1.0 + 0.0*onesfun.x.array
            areaA = fem.assemble_scalar(fem.form(onesfun*dxA(1)))
            areaO = fem.assemble_scalar(fem.form(onesfun*ufl.dx))
            ...
            while t <= T: # t = tn
                ...
                death_tissue.x.array[:] = death_tissue.x.array + tau*Rfactor**(43.0 - (theta.x.array + thetaa))
                I_DT_A     = fem.assemble_scalar(fem.form(death_tissue*dxA(1)))
                I_DT_Omega = fem.assemble_scalar(fem.form(death_tissue*ufl.dx))
                I_DT_Focus = I_DT_Focus + tau*Rfactor**(43.0 - (theta.eval(points_on_proc,cells) + thetaa));
                death_tissue_A = np.hstack((death_tissue_A,  I_DT_A/areaA))
                death_tissue_O = np.hstack((death_tissue_O,  I_DT_Omega/areaO))
                death_tissue_F = np.hstack((death_tissue_F,  I_DT_Focus))
...
scipy.io.savemat( 'SIM/Example2/' + modeltype + '/death_tissue_' + modeltype  + labelin +  '.mat', 
                  { 'death_tissue_A': death_tissue_A, 
                    'death_tissue_O': death_tissue_O, 
                    'death_tissue_F': death_tissue_F} )
